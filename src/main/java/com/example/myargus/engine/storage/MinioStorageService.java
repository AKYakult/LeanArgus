...
public class MinioStorageService implements ObjectStorageService {
     ...
     * @throws BusinessException 合并失败时抛出
     */
    @Override
     ...
     *
     * @param bucket 存储桶名称
     * @throws Exception MinIO SDK 操作失败时抛出
     */
    ...
    /**
     * 从 Spring 环境中安全读取必需配置项。
     *
     * @param environment  Spring 环境对象
     * @param propertyName 配置项完整路径
     * @return 配置项的值
     * @throws BusinessException 配置项缺失或为空时抛出
     */
        ...
        if (!StringUtils.hasText(value)) {
            throw new BusinessException(
                    "对象存储配置缺失: " + propertyName
                            + "，请在 application.yaml / 环境变量中配置 storage.minio.* 相关属性");
        }
        return value;
    }
}
